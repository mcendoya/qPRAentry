...
## 2. Quantity of potentially infested imported commodity ($N_{trade}$)
...
## 3. $N_{trade}$ redistribution from country level to principal subdivisions

The `redist_iso()` function requires an additional data frame with values for each subdivision according to which the redistribution is performed proportionally. The redistribution is shown below using simulated commodity consumption data for each territorial subdivision using ISO codes.
...
In this case, all countries and periods included in the data are taken into account, as the default values are used for the `filter_IDs` and `filter_period` arguments (see example [A](#example-ISO) for other specifications)
...
$N_{trade}$ summary for the time periods (see example [A](#example-ISO) for other specifications).
...
## 3. $N_{trade}$ redistribution from country level (NUTS0) to smaller territorial subdivisions

The `redist_nuts()` function can be used with human population data from Eurotat or from an additional data frame with values for each territorial subdivision according to which the redistribution is performed proportionally. 
