...
Plot $N_{trade}$ median for each country:
...
Note: The `qPRAentry` package currently does not include a built-in function to plot data at subdivision level using ISO 3166-2 codes, although it is available using NUTS codes (see [B.3](#redist-NUTS)). However, users can easily combine the output with other packages, such as `rnaturalearth` and `ggplot2`, to create maps representing these data.

## A.4. Pathway model {#pathway-ISO}

The number of potential founder populations ($NPFP$) of a pest entering a country or region can be estimated using a pathway model. This model combines the $N_{trade}$ data with the parameters that are relevant in the estimation of the entry of the pest under assessment. Each of these parameters must be assigned a suitable probability distribution. The following shows how the $N_{trade}$ data obtained above at the country level are combined with other parameters to set up the pathway model and estimate the $NPFP$.
...
A distribution is then assigned to each parameter and all the information, with the desired number of iterations, is incorporated in the `pathway_model()` function. Note that $N_{trade}$ should not be included in the model expression.
...
Plot $NPFP$ median for each country:
...
Plot $N_{trade}$ mean for each country:
...
The `redist_nuts()` function can be used with human population data from Eurostat or from an additional data frame with values for each territorial subdivision according to which the redistribution is performed proportionally. 
...
Plot $N_{trade}$ mean for each NUTS2 region:
...
Plot $N_{trade}$ mean for each NUTS1:
...
After assigning a distribution to each parameter and specifying the desired number of iterations, it is incorporated along with the model in the `pathway_model()` function. Note that $N_{trade}$ must not be included in the model expression.
