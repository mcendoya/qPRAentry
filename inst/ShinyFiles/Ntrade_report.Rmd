...
In the context of quantitative risk assessment of pest entry, $N_{trade}$ is defined as the potentially infected/infested quantity of commodities imported by a country from 
other third-countries where the pest of interest is present. 
This application is meant to calculate $N_{trade}$ in EU countries (NUTS0) and 
redistribute this quantity to the regions (NUTS2) of the countries considered.
...
quantityis considered as the sum of the internal production of the country ($IP$) 
and the total quantity imported from non-EU countries ($ExtraTotal$), 
...
p <- plot_countries(Nt, "NUTS0", "Median", title = bquote(N[trade] ~ " Median"), legend_title=units) +
...
if(data_redistribution=="Population"){
  ...
  if(length(population_year)==1){
    population_year_text <- paste0("The selected year for population data is ", population_year)
  }else{
    population_year_text <- paste0("The selected years for which the average population will be 
...
plot_nuts(Nt_redist, "NUTS2", "Median", title = bquote(N[trade] ~ " Median"), legend_title=units) +
   xlim(-20, 40) + ylim(35,70) +
   theme_bw()+
  theme(
     plot.title = element_text(size = 14),
