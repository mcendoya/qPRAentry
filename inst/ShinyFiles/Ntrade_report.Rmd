...
p <- plot_nuts(Nt, "NUTS0", "Median",
               nuts_level = 0, nuts_year = nuts_yr,
               title = bquote(N[trade] ~ " Median"), 
               legend_title = units) +
...
if(data_redistribution=="Population"){
  ...
  if(length(population_year)==1){
    population_year_text <- paste0("The selected year for population data is ",
                                   population_year)
  }else{
    population_year_text <- 
      paste0("The selected years for which the average population will be 
...
plot_nuts(Nt_redist, "NUTS2", "Median", 
