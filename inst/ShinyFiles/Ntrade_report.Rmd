...
$N_{trade}$ is defined as the potentially infected/infested quantity of commodities imported by a country (e.g., an EU country) from other third-countries where the pest is present. This application is meant to calculate $N_{trade}$ and redistribute the data into the countries considered.
...
$ExtraPest_i$ and $ExtraPest_j$ are the quantity of commodity imported from non-EU countries where the pest is present by country $i$ and country $j$, respectively. 
$R_{ij}$ and $R_{ji}$ represent the proportion of commodity exported from $i$ to $j$ ($IntraExp_{ij}$), and from $j$ to $i$ ($IntraExp_{ji}$), respectively, out of the total available commodity in the exporter country. This total quantity is considered as the sum of the internal production of the country ($IP$) and the total quantity imported from non-EU countries ($ExtraTotal$), regardless of whether the pest is present. Thus, the ratios $R_{ij}$ and $R_{ji}$ are defined as: 
...
if(length(time_period)==1){
  time_year_text <- paste0("year ", time_period)
  }else{
  time_year_text <- paste0("following years: ", time_period)
}  
  
```
  
When uploading the datasets to calculate $N_{trade}$, the selected unit was `r units`, while the selected time-period included the `r time_year_text` 
...
For this analysis, $N_{trade}$ data were redistributed from NUTS0 to NUTS2 level proportionally to `r data_text`.
...
if(length(population_year)==1){
  ...
  population_year_text <- paste0("years, for which the average population will be used as the basis for redistribution, are: ", population_year)
}
