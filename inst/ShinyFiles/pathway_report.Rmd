...
is estimated for each `r NUTS_lev` level.
The summary of the overall results for all the selected NUTS are shown below.

```{r}

# Display the table
knitr::kable(Ninf_EU, caption = "NPFP results for all the selected NUTS", digits = 2) 
...
```
\begin{center}
Figure 1: $\mathit{NPFP}$ values at the NUTS`r NUTS_lev` level.
\end{center}
