...
other parameters that are involved in the risk of pest entry, from the source country of origin to the countries of interest. Its complexity can vary based on factors 
such as the specific pest under consideration and the availability of detailed data. 
...
As a result of the pathway model the number of $(\mathit{NPFP})$ 
...
`r round(1/Ninf_EU["Median"])` years, with a 90% interval between 
`r round(1/Ninf_EU["Q0.05"])` and `r round(1/Ninf_EU["Q0.95"])` years.
...
```
\begin{center}
Figure 1: Number of potential founder populations ($\mathit{NPFP}$)
at the NUTS`r NUTS_lev` level.
\end{center}
